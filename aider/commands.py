...
class Commands:
    ...
    def cmd_chat_mode(self, args):
        ...
        show_formats = OrderedDict(
            [
                ("research", "Ask research questions about any topics."),
    ...
    def cmd_tokens(self, args):
        ...
        self.io.tool_output(
            f"Approximate context window usage for {
                self.coder.main_model.name
            }, in tokens:"
        )
        self.io.tool_output()
        ...
        if remaining > 1024:
            ...
            self.io.tool_error(
                f"{cost_pad}{
                    fmt(remaining)
                } tokens remaining in context window (use /drop or"
                " /clear to make space)"
            )
        else:
            self.io.tool_error(
                f"{cost_pad}{
                    fmt(remaining)
                } tokens remaining, window exhausted (use /drop or"
                " /clear to make space)"
            )
    ...
    def raw_cmd_undo(self, args):
        ...
        if len(last_commit.parents) > 1:
            self.io.tool_error(
                f"The last commit {
                    last_commit.hexsha
                } has more than 1 parent, can't undo."
            )
            return
        ...
        for fname in changed_files_last_commit:
            if self.coder.repo.repo.is_dirty(path=fname):
                self.io.tool_error(
                    f"The file {
                        fname
                    } has uncommitted changes. Please stash them before undoing."
                )
                return
            ...
            try:
                ...
                self.io.tool_error(
                    f"The file {
                        fname
                    } was not in the repository in the previous commit. Cannot"
                    " undo safely."
                )
                return
    ...
    def cmd_add(self, args):
        ...
        for word in filenames:
            ...
            if "*" in str(fname) or "?" in str(fname):
                self.io.tool_error(
                    f"No match, and cannot create file with wildcard characters: {
                        fname
                    }"
                )
                continue
        ...
        for matched_file in sorted(all_matched_files):
            ...
            if not abs_file_path.startswith(self.coder.root) and not is_image_file(
                ...
                self.io.tool_error(
                    f"Can not add {abs_file_path}, which is not within {
                        self.coder.root
                    }"
                )
                continue
            ...
            if abs_file_path in self.coder.abs_fnames:
                ...
                if self.coder.repo and self.coder.repo.path_in_repo(matched_file):
                    ...
                    self.io.tool_output(
                        f"Moved {
                            matched_file
                        } from read-only to editable files in the chat"
                    )
                else:
                    self.io.tool_error(
...
"""
    ...
    def cmd_research(self, args):
        """Ask research questions about any topics. If no prompt provided,switches to research mode."""
        return self._generic_chat_command(args, "research")
...
"""
    ...
    def _add_read_only_directory(self, abs_path, original_name):
        ...
        if added_files > 0:
            self.io.tool_output(
                f"Added {added_files} files from directory {
                    original_name
                } to read-only files."
            )
        else:
    ...
    def cmd_think_tokens(self, args):
        ...
        if not args.strip():
            ...
            if formatted_budget is None:
                ...
                self.io.tool_output(
                    f"Current thinking token budget: {budget:,} tokens ({
                        formatted_budget
                    })."
                )
            return
